        ...
        <form action="<?= url('/admin/insights/edit/' . $blog['id']) ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="tags" value="<?= e($blog['tags'] ?? '') ?>" placeholder="e.g. Technology, Security, ERP" class="w-full bg-slate-50 border border-slate-200 focus:border-indigo-500 rounded-xl px-4 py-3 text-xs outline-none transition-all text-slate-850">
                </div>
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1.5">Add Gallery Images (Optional)</label>
            ...
            <div>
                ...
                <textarea name="quote" rows="2" placeholder="An impactful quote to highlight within the article..." class="w-full bg-slate-50 border border-slate-200 focus:border-indigo-500 rounded-xl px-4 py-3 text-xs outline-none transition-all text-slate-850"><?= e($blog['quote'] ?? '') ?></textarea>
            </div>
