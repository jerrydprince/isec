...
<body class="bg-slate-100 font-sans text-slate-800 antialiased py-10">
    ...
    <div class="max-w-4xl mx-auto bg-white shadow-lg print-border rounded-xl p-10 sm:p-16 mb-10">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start gap-6 border-b border-slate-200 pb-8">
            <div>
                <!-- You can replace this text with an img tag for the logo -->
            ...
            <div class="text-left sm:text-right">
                ...
                <div class="text-xs text-slate-500 mt-1">Date: <?= date('F j, Y', strtotime($invoice['issue_date'])) ?></div>
                <?php if ($invoice['due_date']): ?>
                    <div class="text-xs text-slate-500 mt-1">Due Date: <?= date('F j, Y', strtotime($invoice['due_date'])) ?></div>
                <?php endif; ?>
                
                <div class="mt-4 inline-block px-3 py-1 rounded text-xs font-bold uppercase tracking-wider
