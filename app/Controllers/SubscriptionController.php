...
class SubscriptionController extends AdminController {
    ...
    public function index(Request $request, Response $response): string {
        ...
        $customTemplates = $db->query("SELECT * FROM message_templates ORDER BY name ASC")->fetchAll();
    ...
    public function sendReminder(Request $request, Response $response): void {
        ...
        if (in_array('Email', $channels) && !empty($sub['client_email'])) {
            if (is_numeric($templateType)) {
                $customTpl = MessageTemplate::find($templateType);
                $body = $customTpl ? strtr($customTpl['body'], $replacements) : "Reminder for {service_name}";
                $subject = $customTpl ? strtr($customTpl['subject'] ?? "Subscription Reminder", $replacements) : "Subscription Reminder";
            } else {
